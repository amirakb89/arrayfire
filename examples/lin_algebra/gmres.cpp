...
#include <iostream>
using namespace af;
constexpr int num_test =5;
struct info_solver {
  std::vector<int> iter;
  std::vector<float> time;
  std::vector<float> nnz;
info_solver(int num_test){
  iter.reserve(num_test);
  time.reserve(num_test);
  nnz.reserve(num_test);
}
};
template <class T>
...
static void solve_demo(dtype dt, info_solver &solve_info) { 
  ...
  */
  ...
  for (int n = 20; n<max_n; n*=2){
    ...
    solve_info.iter.push_back(static_cast<int> (num_iter));
